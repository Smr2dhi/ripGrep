...
def get_context( grep_result, context_lines=5, max_chars=12000):
    if not grep_result:
    ...
    for file_path, line_numbers in matches.items():
        ...
        try:
            ...
            logger.error( "File not found while retrieving context: %s", full_path )

            continue

        except Exception as e:
            logger.exception( "Could not read file: %s",full_path)
        ...
        logger.info("Created %s context groups for %s",len(ranges),file_path)
        ...
        for start, end in ranges:
            ...
            for line_number in range(context_start,context_end + 1):
                ...
                item = ( f"{file_path}:{line_number}:{text}")
        ...
        for item in file_context:
            if total_chars + len(item) > max_chars:
                ...
                logger.warning("Maximum context size reached: %s characters",max_chars)
    ...
    logger.info(
        "Context retrieval completed - %s characters returned",total_chars)
